...
while true
    ...
    [min_dist, current] = min(distanceFromStart(:)) % min_dist stores the minimum value and current stores the index of minimum distance
   ...
   for nums=1:4
      ...
      if a~=0
       if (map(a)== 1 || map(a)== 4 || map(a)== 6)
          if distanceFromStart(a)> min_dist + 1
             distanceFromStart(a) = min_dist + 1;
             parent(a) = current;
             map(a) = 4;
       end
    end
